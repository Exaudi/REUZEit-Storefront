...
class Payment_Form extends Base_Payment_Form {
	...
	/** @var string[] Flex key generation JWK array */
	protected $jwk;
	...
	public function render_payment_field( $field ) {
		...
		if ( isset( $field['id'] ) && 'wc-cybersource-credit-card-account-number' === $field['id'] ) {
	...
	private function get_flex_microform_key() {
		...
		if ( empty( $this->jwk ) ) {
			...
			try {
				...
				$this->jwk = $this->get_gateway()->get_api()->generate_public_key()->get_jwk_array();
	...
	protected function get_js_handler_args() {
		...
		if ( ! empty( $jwk = $this->get_flex_microform_key() ) ) {
	...
	public function render_js() {
		...
		if ( ! empty( $this->get_flex_microform_key() ) ) {
			...
			$url = $this->get_gateway()->is_production_environment() ? 'https://flex.cybersource.com/cybersource/assets/microform/0.4/flex-microform.min.js' : 'https://testflex.cybersource.com/cybersource/assets/microform/0.4/flex-microform.min.js';
